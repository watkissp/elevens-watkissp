...
public class ElevensBoard extends Board {
    ...
    public boolean isLegal(List<Integer> selectedCards) {
        ...
        //if there are 2 cards... return contains pairsum11
        if (selectedCards.size() == 2) {
            return containsPairSum11(selectedCards);
        } // else if there are 3 cards... return contains jqk
    ...
    public boolean anotherPlayIsPossible() {
        /* *** TO BE IMPLEMENTED IN ACTIVITY 9 *** */
        // get all cards that are still on board
    ...
    private boolean containsPairSum11(List<Integer> selectedCards) {
        ...
        for (int x = 0; x < selectedCards.size(); x++) {
            int outer = selectedCards.get(x).intValue();
            Card first = this.cardAt(outer);

            //inner loop
            for (int y = 0; y < selectedCards.size(); y++) {
                int inner = selectedCards.get(y).intValue();
                Card second = this.cardAt(inner);

                //if outer card + inner card == 11
                if (outer != inner && 
                        first.pointValue() + second.pointValue() == 11) {
                    return true;
                }
            }
        }
